...
class Command(TemplateCommand):
    ...
    def handle(self, **options):
        ...
        template_dir = self.handle_template(options['template'], 'appi_template')
        ...
        for root, dirs, files in os.walk(template_dir):
            ...
            if relative_dir:
                ...
                if not path.exists(target_dir):
                    os.mkdir(target_dir)
                    if relative_dir == 'templates':
                        target_dir_template = path.join(target_dir, app_name)
                        os.mkdir(target_dir_template)
        ...
        if self.verbosity >= 1:
            self.stdout.write(self.style.SUCCESS(f"{camel_case_app_name} app created"))
            self.stdout.write(self.style.SQL_COLTYPE(
                f"To enable the module you need to add 'apps.{ app_name }.apps.{camel_case_app_name}Config' "
